...
// Enhanced logging function with data verification
function logTestResult(testName, status, statusCode, response, validation) {
  ...
  // Real-time logging with enhanced data display
  if (status === "PASS") {
...
// New function to log generated data for UI verification
function logGeneratedData(dataType, data, additionalInfo = '') {
  const timestamp = new Date().toISOString();
  console.log(`\n🔍 ${dataType.toUpperCase()} DATA GENERATED (${timestamp}):`);
...
// New function to log test data summary
function logTestDataSummary() {
  console.log('\n📊 CURRENT TEST DATA SUMMARY:');
  console.log('='.repeat(60));
  console.log(`🔑 Access Token: ${testData.accessToken ? '✅ Present' : '❌ Missing'}`);
  console.log(`👨‍⚕️ Provider UUID: ${testData.providerUUID || '❌ Not set'}`);
  console.log(`👤 Patient UUID: ${testData.patientUUID || '❌ Not set'}`);
  console.log(`📧 Provider Email: ${testData.providerEmail || '❌ Not set'}`);
  console.log(`👨‍⚕️ Provider Name: ${testData.providerFirstName || '❌ Not set'} ${testData.providerLastName || '❌ Not set'}`);
  console.log(`📧 Patient Email: ${testData.patientEmail || '❌ Not set'}`);
  console.log(`👤 Patient Name: ${testData.patientFirstName || '❌ Not set'} ${testData.patientLastName || '❌ Not set'}`);
  console.log(`📅 Appointment UUID: ${testData.appointmentUUID || '❌ Not set'}`);
  console.log(`🕐 Available Slots: ${testData.availableSlots ? (Array.isArray(testData.availableSlots) ? testData.availableSlots.length : 'Present') : '❌ Not set'}`);
  console.log('='.repeat(60));
}
...
function generateRandomData() {
  ...
  logGeneratedData('Random Test Data', generatedData, 'Generated for test execution');
...
function getAppointmentSlotTimes(weeksAhead = 0, dayName = 'MONDAY', hour = 14) {
  ...
  // Log slot times generation for verification
  logGeneratedData('Appointment Slot Times Generated', {
    weeksAhead: weeksAhead,
    dayName: dayName,
    hour: hour,
    date: slotTimes.date,
    localStartTime: slotTimes.localStartTime.toLocaleString(),
    localEndTime: slotTimes.localEndTime.toLocaleString(),
    utcStartTime: slotTimes.utcStartTime,
    utcEndTime: slotTimes.utcEndTime
  }, `Generated slot times for ${dayName} ${weeksAhead} weeks ahead at ${hour}:00`);
...
async function bookAppointment(request, appointmentData) {
  ...
  try {
    ...
    // Log booking attempt result
    logGeneratedData('Appointment Booking Attempt Result', {
      statusCode: statusCode,
      success: statusCode === 200 || statusCode === 201,
      appointmentUUID: responseData.data?.uuid || 'Not provided',
    ...
    }, `Booking attempt completed with status ${statusCode}`);
    ...
    // Log booking error
    logGeneratedData('Appointment Booking Error', {
      error: error.message,
      patientId: appointmentData.patientId,
      providerId: appointmentData.providerId,
      startTime: appointmentData.startTime,
      endTime: appointmentData.endTime
    }, 'Booking attempt failed with error');
...
test.describe('eCareHealth API End-to-End Test Suite - FINAL FIXED', () => {
  ...
  test('Complete API Workflow - ALL ISSUES FIXED', async ({ request }) => {
    ...
    try {
      ...
      // Log login data for verification
      logGeneratedData('Login Response', {
        statusCode: statusCode,
        accessToken: testData.accessToken ? '✅ Present' : '❌ Missing',
        userInfo: loginData.data.user || 'Not provided',
    ...
    try {
      ...
      // Log provider data being created
      logGeneratedData('Provider Creation Data', providerData, 'Provider data being sent to API');
      ...
      if (statusCode === 200 || statusCode === 201 || 
        ...
        logGeneratedData('Provider Creation Response', {
          statusCode: statusCode,
          providerUUID: providerResponseData.data?.uuid || 'Not provided',
          providerName: `${testData.providerFirstName} ${testData.providerLastName}`,
          providerEmail: testData.providerEmail,
          message: providerResponseData.message || 'Success'
    ...
    try {
      ...
      logGeneratedData('Provider Details Retrieved', {
        providerUUID: testData.providerUUID,
        providerName: `${testData.providerFirstName} ${testData.providerLastName}`,
        providerEmail: foundProvider.email || 'Not provided',
        providerStatus: foundProvider.status || 'Unknown',
        providerActive: foundProvider.active || false,
        totalProvidersFound: providerListData.data?.content?.length || 0
      }, 'Provider details retrieved from API');
    ...
    try {
      ...
      // Log availability data being set
      logGeneratedData('Availability Settings Data', availabilityData, 'Availability settings being sent to API');
      ...
      if (statusCode === 200 || statusCode === 201 || 
        ...
        // Log successful availability setup
        logGeneratedData('Availability Setup Response', {
          statusCode: statusCode,
          providerId: testData.providerUUID,
          message: availabilityResponseData.message || 'Success',
          timezone: "EST",
          bookingWindow: "14 days",
          daySlotsCount: availabilityData.daySlots.length
        }, 'Availability settings applied successfully');
    ...
    try {
      ...
      // Log patient data being created
      logGeneratedData('Patient Creation Data', patientData, 'Patient data being sent to API');
      ...
      if (statusCode === 200 || statusCode === 201 || 
        ...
        logGeneratedData('Patient Creation Response', {
          statusCode: statusCode,
          patientUUID: patientResponseData.data?.uuid || 'Not provided',
          patientName: `${testData.patientFirstName} ${testData.patientLastName}`,
          patientEmail: testData.patientEmail,
          patientPhone: patientTestData.phone,
          message: patientResponseData.message || 'Success'
    ...
    try {
      ...
      logGeneratedData('Patient Details Retrieved', {
        patientUUID: testData.patientUUID,
        patientName: `${testData.patientFirstName} ${testData.patientLastName}`,
        patientEmail: foundPatient.email || 'Not provided',
        patientPhone: foundPatient.mobileNumber || 'Not provided',
        patientStatus: foundPatient.status || 'Unknown',
        totalPatientsFound: patientListData.data?.content?.length || 0
      }, 'Patient details retrieved from API');
    ...
    try {
      ...
      if (slotsFound && bestSlotResult) {
        // Log available slots data for verification
        logGeneratedData('Available Slots Data', {
          statusCode: bestSlotResult.statusCode,
          endpoint: bestSlotResult.endpoint?.split('?')[0] || 'Unknown',
          providerId: testData.providerUUID,
          providerName: `${testData.providerFirstName} ${testData.providerLastName}`,
          slotsCount: Array.isArray(bestSlotResult.data) ? bestSlotResult.data.length : 'N/A',
          slotsData: Array.isArray(bestSlotResult.data) ? bestSlotResult.data.slice(0, 3) : bestSlotResult.data,
          timezone: 'EST'
    ...
    try {
      ...
      if (testData.availableSlots && Array.isArray(testData.availableSlots) && testData.availableSlots.length > 0) {
        ...
        // Log appointment booking data
        logGeneratedData('Appointment Booking Data (Strategy 1)', appointmentData, 'Appointment data being sent to API using available slot');
      ...
      if (!appointmentBooked) {
        ...
        // Log calculated appointment data
        logGeneratedData('Appointment Booking Data (Strategy 2)', calculatedAppointmentData, 'Appointment data being sent to API using calculated time');
      ...
      if (!appointmentBooked) {
        ...
        for (const timeSlot of timeSlots) {
          ...
          // Log multiple time slot appointment data
          logGeneratedData(`Appointment Booking Data (Strategy 3 - ${timeSlot.label})`, multiTimeAppointmentData, `Appointment data being sent to API for ${timeSlot.label}`);
      ...
      if (appointmentBooked && bookingResult) {
        ...
        logGeneratedData('Appointment Booking Success', {
          statusCode: bookingResult.statusCode,
          appointmentUUID: testData.appointmentUUID || 'Not provided',
          patientName: `${testData.patientFirstName} ${testData.patientLastName}`,
          providerName: `${testData.providerFirstName} ${testData.providerLastName}`,
          message: bookingResult.data.message || 'Success',
          bookingStrategy: 'Multiple strategies attempted'
    ...
    // Log comprehensive final summary for UI verification
    logGeneratedData('FINAL TEST SUMMARY', {
      ...
      errorTests: report.summary.errors,
      provider: {
        uuid: testData.providerUUID,
        name: `${testData.providerFirstName} ${testData.providerLastName}`,
        email: testData.providerEmail
      },
      patient: {
        uuid: testData.patientUUID,
        name: `${testData.patientFirstName} ${testData.patientLastName}`,
        email: testData.patientEmail
      },
      appointment: {
        uuid: testData.appointmentUUID || 'Not created',
        status: testData.appointmentUUID ? 'Created' : 'Failed'
      },
      availableSlots: {
        count: Array.isArray(testData.availableSlots) ? testData.availableSlots.length : 'N/A',
        found: testData.availableSlots ? 'Yes' : 'No'
      },
      accessToken: testData.accessToken ? 'Present' : 'Missing'
    }, 'Complete test execution summary for UI verification');
  });
});
